"""Tests verifying that 2023-2 Science and Mathematics detailed explanations are properly assembled and queryable."""
...
def test_database_has_reviewed_explanations(live_db):
    """The explanations are still attached to the release they were written for.

    They were authored against a version that has since been suspended — its
    answers were placeholders — so this looks for the newest version that
    *carries explanations* rather than the newest version outright. Asserting on
    the newest outright would fail on every fresh release, and would quietly
    claim the retired content is what ships.
    """
    papers = [
        ("eju-2023-2-math-c1-ja", 8),
        ("eju-2023-2-math-c2-ja", 8),
        ("eju-2023-2-science-ja", 57),
    ]
...
def test_work_paper_json_explanations():
    """Verify that paper.json files in work/ have explanation payloads matching the contract."""
    sessions = [
        ("work/2023-2-math-c1/paper.json", 8),
        ("work/2023-2-math-c2/paper.json", 8),
        ("work/2023-2-science/paper.json", 57),
    ]
...
def test_review_content_workflow(live_db):
    ...
    ``eju-2023-2-science-ja`` and its explanations read back. That paper is one of
    ...
    ``test_work_paper_json_explanations``.
    """
    row = live_db.connection.execute(
        "SELECT id FROM papers WHERE stable_code = 'eju-2023-2-science-ja'").fetchone()
