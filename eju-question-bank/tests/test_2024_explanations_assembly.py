"""Tests verifying that 2024 Science and Mathematics detailed explanations are properly assembled and queryable."""
...
def test_2024_database_has_reviewed_explanations(live_db):
    """Verify that all 73 questions across Math 1, Math 2, and Science have reviewed explanations in DB for 2024."""
    papers = [
        ("eju-2024-1-math-c1-ja", 8),
        ("eju-2024-1-math-c2-ja", 8),
        ("eju-2024-1-science-ja", 57),
    ]
    ...
    for stable_code, expected_count in papers:
        ...
        assert len(q_rows) == expected_count, (
            f"Expected {expected_count} questions for {stable_code}, found {len(q_rows)}"
        )
        ...
        for q_row in q_rows:
            ...
            # Explanation revision check
            exp_row = live_db.connection.execute(
                """
                SELECT status, payload_json, reviewer
            ...
            assert exp_row is not None, f"Missing explanation revision for {stable_code}:{skey}"
            assert exp_row["status"] == "REVIEWED", f"Explanation for {stable_code}:{skey} is not REVIEWED"

            payload = json.loads(exp_row["payload_json"])
            assert payload["kind"] == "EXPLANATION"
            assert payload["language"] == "zh"
            assert payload["title"], f"Empty title in {stable_code}:{skey}"
            assert len(payload["points"]) > 0, f"Empty points in {stable_code}:{skey}"
...
def test_2024_work_paper_json_explanations():
    """Verify that paper.json files in work/ for 2024 have explanation payloads matching the contract."""
    sessions = [
        ("work/2024-1-math-c1/paper.json", 8),
        ("work/2024-1-math-c2/paper.json", 8),
        ("work/2024-1-science/paper.json", 57),
    ]
...
def test_2024_review_content_workflow(live_db):
    ...
    ``eju-2024-1-science-ja`` and its explanations read back. That paper is one of
    ...
    ``test_2024_work_paper_json_explanations``.
    """
    row = live_db.connection.execute(
        "SELECT id FROM papers WHERE stable_code = 'eju-2024-1-science-ja'").fetchone()
...
def test_2024_docs_exist():
    """Verify all 5 Markdown explanation solution files exist and have substantial content."""
    docs = [
        "docs/explanations/2024-physics-solutions.md",
        "docs/explanations/2024-chemistry-solutions.md",
        "docs/explanations/2024-biology-solutions.md",
        "docs/explanations/2024-math-c1-solutions.md",
        "docs/explanations/2024-math-c2-solutions.md",
    ]
    for d in docs:
        p = PROJECT_ROOT / d
